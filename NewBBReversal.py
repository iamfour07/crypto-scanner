...
def get_market_sentiment(stats):
    """
    Determines overall market direction based on the net percentage difference.
    """
    total_coins = len(stats)
    if total_coins == 0:
        return "⚠️ Market Status: No data available."
    ...
    # Calculate the Net Difference as a Percentage of the total market
    # Example: (60 gainers - 40 losers) / 100 total = +20% Net Difference
    diff = num_gainers - num_losers
    net_diff_pct = (diff / total_coins) * 100
    ...
    msg = (
        f"📊 **MARKET STATUS REPORT**\n"
        f"Status: **{status}**\n\n"
        f"⚖️ Net Difference: {net_diff_pct:+.1f}%\n\n"
        f"Note: {advice}"
    )
    return msg
...
def calculate_indicators(df):
    ...
    # MACD WMA
    fast_ma = calculate_wma(df['close'], MACD_FAST)
    slow_ma = calculate_wma(df['close'], MACD_SLOW)
    df['macd_line'] = fast_ma - slow_ma
    df['macd_signal'] = calculate_wma(df['macd_line'], MACD_SIGNAL)
...
def process_logic(pair, watch_list):
    ...
    if pair in watch_list:
        # Debugging Print
        # print(f"DEBUG [{pair}] | MACD: {last['macd_line']:.6f}/{last['macd_signal']:.6f} | Price: {last['close']} (BB_UP: {last['BB_upper']:.6f})")

        # Confirm: MACD bearish AND Price returned inside BB Upper
        if last['macd_line'] < last['macd_signal'] and last['close'] < last['BB_upper']:
            entry = last["close"]
            sl = max(last["high"], prev["high"])
            risk = sl - entry
            ...
            return {
                "type": "SIGNAL", "pair": pair, "entry": entry, "sl": sl, 
                "margin": margin, "rsi": round(last['rsi'], 2),
...
def main():
    ...
    # --- 2. SCANNING ---
    # Scan watchlist + top 30 gainers for discovery
    top_gainers = [x["pair"] for x in sorted(stats, key=lambda x: x["change"], reverse=True)[:30]]
    scan_pool = list(set(watch_list + top_gainers))

    alerts, new_watchlist, signaled_pairs = [], [], []

    print(f"--- Starting Scan ({len(scan_pool)} pairs) ---")
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        tasks = [executor.submit(process_logic, p, watch_list) for p in scan_pool]
        for f in as_completed(tasks):
            res = f.result()
            ...
            if res["type"] == "SIGNAL":
                msg = (
                    f"🔴 **MACD REVERSAL SELL**: {res['pair']}\n"
                    f"Entry: {res['entry']:.6f} | SL: {res['sl']:.6f}\n"
                    f"Capital: ₹{res['margin']:.2f} | RSI: {res['rsi']}\n"
